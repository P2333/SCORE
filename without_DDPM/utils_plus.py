...
def attack_pgd(model, X, y, epsilon, alpha, attack_iters, restarts, norm, use_CWloss=False):
    bs = y.shape[0]
...
def evaluate_pgd(test_loader, model, attack_iters, restarts, eps=8, step=2, norm='linf', use_CWloss=False):
    epsilon = eps / 255.
    alpha = step / 255.
    pgd_loss = 0
    pgd_acc = 0
    ...
    for i, (X, y) in enumerate(test_loader):
        X, y = X.cuda(), y.cuda()
        pgd_delta = attack_pgd(model, X, y, epsilon, alpha, attack_iters, restarts, norm, use_CWloss=use_CWloss)
        with torch.no_grad():
...
def evaluate_random_noise(test_loader, model, noise_scale=1):
    test_loss = 0
    test_acc = 0
    n = 0
    model.eval()
